...
**Spell**是一个**Brainfuck-like**的简易编程语言，以图灵机模型为基础提供了多个指令，除了拥有Brainfuck所提供的操作外，还添加了数个方便的运算指令以及一套寄存器。由于以其编写的程序晦涩难懂，因此得名Spell，即**咒语**:magic_wand:。
...
| qox    | 偏移量寄存器，设置bit位等操作时由其提供偏移量    | o    |          |
| qpx    | 指针寄存器，指向当前操作的内存单元               | p    |          |
| qrx    | 结果寄存器，保存某些运算结果                     | r    |          |
| qtx    | 临时寄存器，参与某些运算和数据交换               | t    |          |
...
| 指令 | 作用                                    |
| ---- | --------------------------------------- |
| [    | 当寄存器qcx为0时，前跳到另一侧的]之后   |
| ]    | 当寄存器qcx不为0时，回跳到另一侧的[之后 |
